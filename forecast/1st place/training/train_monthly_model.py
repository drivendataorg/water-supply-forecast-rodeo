...
def train_monthly_model(model_dir: Path,
    ...
    def monthly_null_prev_month_nat_flow(rw):
        if (pd.to_datetime(rw['issue_date']).day == 1) and (rw['month'] - rw['pred_month'] == 1):
            rw['prev_month_volume'] = np.nan

        return rw

    if null_monthly_nat_flow:
        train_features = train_features.apply(lambda rw: monthly_null_prev_month_nat_flow(rw), axis=1)

    label = 'month_volume_log'

    feature_cols = ['site_id', 'pred_month', 'latitude', 'longitude', 'elevation', 'elevation_stds',
                    'day_of_year', 'precip_deviation_30_mean', 'streamflow_deviation_30_mean',
                    'maxt_deviation_30_mean', 'combined_swe_deviation_30', 'ua_swe_deviation', 'acc_water_deviation']
