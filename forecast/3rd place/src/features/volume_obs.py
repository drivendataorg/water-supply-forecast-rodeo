...
def prepare_volume_obs(
    df_meta, usbr_dir="data/external/usbr", usgs_dir=USGS_DIR, usgs_format="rdb"
):
    ...
    df_usbr = df_usbr.groupby(["site_id", "date"], as_index=False).agg(
        volume_obs=("volume_obs", "sum")
    )

    if usgs_format == "rdb":
        df_usgs = read_discharge(
            "data/external/usgs_streamflow/raw_1900-01-01_2023-10-21_daily.txt"
        )
...
def prepare_volume_obs_diff(df_volume_obs, df_meta):
    ...
    df_volume_obs_filtered = df_volume_obs_filtered.assign(
        volume_obs_cml=lambda x: x.groupby(["site_id", "wyear"])[
            "volume_obs"
        ].transform("cumsum"),
        volume_obs_month_cml=lambda x: x.groupby(["site_id", "wyear", "month"])[
            "volume_obs"
        ].transform("cumsum"),
    )
    ...
    df_volume_obs_filtered.loc[
        df_volume_obs_filtered["day"] == 1, "volume_obs_month_cml_lag1"
    ] = np.NaN
...
def get_volume_target_diff_extra(
    ...
    df_train_base = pd.merge(
        df_train_base,
        df_volume_obs_filtered.drop(
            columns=["date", "yday", "volume_obs_cml", "volume_obs_month_cml"]
        ),
        how="left",
    )
    df_train_base["volume_obs_cml_lag1"] = df_train_base["volume_obs_cml_lag1"].fillna(
        0
    )
    df_train_base["volume_obs_month_cml_lag1"] = df_train_base[
        "volume_obs_month_cml_lag1"
    ].fillna(0)
    ...
    df_train_base = df_train_base.drop(
        columns=["volume_obs_cml_lag1", "volume_obs_month_cml_lag1"] + remove_cols
    )
