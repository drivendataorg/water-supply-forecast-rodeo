...
def read_train(dirname=RAW_DATA_DIR, meta=None, test_years=None, is_forecast=False):
    if is_forecast:
        df = pd.read_csv(f"{dirname}/forecast_train.csv")
...
def read_monthly_naturalized_flow(cats=["train", "test"], dirname=RAW_DATA_DIR, is_forecast=False):
    if is_forecast:
        df = pd.read_csv(f"{dirname}/forecast_train_monthly_naturalized_flow.csv")
    else:
        df = []
        for cat in cats:
