...
def agg_error_metrics(
    df, y_actual="volume", y_pred=["pred_volume_10", "pred_volume_50", "pred_volume_90"]
):
    y_pred_10 = y_pred[0]
    y_pred_90 = y_pred[2]
    y_pred = y_pred[1]
    ...
    int_cvr = (
        np.sum(
            np.where(
                (df[y_actual] > df[y_pred_10]) & (df[y_actual] < df[y_pred_90]), 1, 0
            )
        )
        / n
    )
...
def eval_all(pred_df, grouper_list=[["year"]], html=True, is_include_mean_std=True):
    for grouper in grouper_list:
        if html:
            ...
            display(
                eval_agg(
                    pred_df, grouper=grouper, is_include_mean_std=is_include_mean_std
                )
            )
            print("\n")
        else:
...
def generate_hindcast_submission(
    df_pred, df_sub=read_sub(), fname=None, dirname="data/sub/hindcast"
):
    df_sub = pd.merge(
