...
def preprocess_pdsi(dirname, df_meta_poly, out_file=None):
    pdsi_files = glob.glob(f"{dirname}/FY**/*.nc", recursive=True)
    ...
    for file in pdsi_files:
        ...
        for idx, row in df_meta_poly.iterrows():
            _res = ds.rio.clip([row["geometry"]]).mean(("lon", "lat"))
            _res = _res.to_dataframe()[["daily_mean_palmer_drought_severity_index"]].reset_index()
