...
class TimeSeriesSample:
    ...
    def filter_by_forecast_doy(self, forecast_doy: Union[int, List[int], np.ndarray]):
        ...
        if self.snotel_agg_water is not None:
            mask = self.snotel_doy < forecast_doy[2]
        ...
        if self.cdec_snowwater is not None:
            mask = self.cdec_doy < forecast_doy[3]
            sample.cdec_snowwater = self.cdec_snowwater[mask]
            sample.cdec_doy = self.cdec_doy[mask]
