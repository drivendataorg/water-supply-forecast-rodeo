...
def read_train(path=f"{RAW_DATA_DIR}/train.csv", meta=None, test_years=None):
    ...
    if (test_years is not None) & (meta is not None):
        df = pd.concat([df, generate_base_year(meta, years=test_years)]).reset_index(
            drop=True
        )
...
def read_usbr_all(
    dirname="data/external/usbr", parameter="Lake/Reservoir Inflow", unit="cfs"
):
...
def read_snotel_swe(dirname, site_list=None):
    ...
    if site_list is None:
        site_list = [
            os.path.basename(x).replace(".csv", "").split("_")[0] for x in swe_files
        ]
