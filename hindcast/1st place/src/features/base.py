...
def parse_time_features(
    df, cols=["date"], level=["year", "month", "day", "yday", "wyear", "wyday"]
):
    df = df.copy()
    for col in cols:
        ...
        if "wyear" in level:
            df["wyear"] = df[col].dt.year.where(
                df[col].dt.month < 10, df[col].dt.year + 1
            )
        if "wyday" in level:
            df["wyday"] = (
                df[col] - pd.to_datetime((df["wyear"] - 1).astype("str") + "-09-30")
            ).dt.days
...
def haversine(
    lat1, lon1, lat2, lon2, to_radians=True, to_miles=False, earth_radius=6371
):
...
def generate_base_forecast(df_meta, years=[2024]):
    ...
    for year in years:
        ...
        df_base = pd.merge(df_meta[["site_id"]].assign(a=0, year=year), df_base).drop(
            columns=["a"]
        )
        df_base_all.append(df_base)
    df_base_all = pd.concat(df_base_all).reset_index(drop=True)
...
def expand_date(df, date_col="date", groupby_cols=["site_id"], max_mm_dd=None):
    df = df.copy()
    ...
    df = pd.merge(
        df_base.assign(a=0),
        df.assign(date=lambda x: pd.to_datetime(x["date"]), a=0),
        how="left",
    ).drop(columns=["a"])
...
def generate_synthetic_data(
    ...
    scale_factor = np.random.uniform(
        low=scale_factor[0], high=scale_factor[1], size=(len(train_year), n_synthetic)
    )
...
def generate_target_diff(df_monthly, df_meta):
    ...
    df_target_diff = (
        ...
        .assign(
            diff=lambda x: x.groupby(["site_id", "year"])["volume"].transform("cumsum")
...
def get_month_tf(df, diff_gap=1):
    ...
    df = df.copy()
    df = df.assign(
        month_tf=lambda x: np.where(x["day"] <= diff_gap, x["month"] - 1, x["month"])
    )
...
def forward_fill(df, groupby_cols):
    """
...
def get_meta_features(df, df_meta):
    ...
    df = pd.merge(
        ...
        df_meta[
            ...
            n_months=lambda x: x["season_end_month"] - x["season_start_month"] + 1
        ),
    )
