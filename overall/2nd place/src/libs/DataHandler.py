...
class DataHandler:
    ...
    def train_data(self) -> pd.DataFrame:
        df_cv = pd.read_csv(os.path.join(self.base_dir, 'final_stage', 'cross_validation_labels.csv')).dropna()
        df_prev = pd.read_csv(os.path.join(self.base_dir, 'final_stage', 'prior_historical_labels.csv')).dropna()
    ...
    def metadata(self) -> pd.DataFrame:
        df = pd.read_csv(os.path.join(self.base_dir, 'final_stage', 'metadata.csv'))
        return df

    @cache
    def train_monthly_flow(self) -> pd.DataFrame:
        df_cv = pd.read_csv(os.path.join(self.base_dir, 'final_stage', 'cross_validation_monthly_flow.csv')).dropna()
        df_prev = pd.read_csv(os.path.join(self.base_dir, 'final_stage', 'prior_historical_monthly_flow.csv')).dropna()
        df = pd.concat([df_prev, df_cv])
        return df
    ...
    @cache
    def test_monthly_flow_normalized(self, test_df: pd.DataFrame) -> pd.DataFrame:
        return self.scaler.transform_df(test_df)
    ...
    @cache
    def final_kfold_train_test_splits(self) -> List[Tuple[np.ndarray, int]]:
        splits = []
        all_years = np.unique(self.train_data()['year'].values)
