...
def train_lgb(
    ...
    train_data = lgb.Dataset(df[train_mask][all_features], label=df[train_mask][cfg["target"]])
    logger.info("Train data frame size: ({}, {})".format(len(train_mask[train_mask]), len(all_features)))

    if not is_full_train:
        valid_mask = df["cat"] == "val"
        valid_data = lgb.Dataset(df[valid_mask][all_features], label=df[valid_mask][cfg["target"]])
        logger.info("Validation year {}".format(df[valid_mask]["year"].unique()))
        logger.info("Validation data frame size: ({}, {})".format(len(valid_mask[valid_mask]), len(all_features)))
    ...
    estimators = []
    df_eval_scores = []
    df_pred = df[~train_mask][["site_id"] + cfg["remove_features"] + ["month", "day"]].copy()
    for quantile in quantiles:
        ...
        estimator = lgb.train(
            ...
            valid_sets=[train_data, valid_data] if (not is_full_train) else [train_data],
            verbose_eval=verbose,
        )
        df_eval_score = (
            pd.DataFrame({"train": evals["training"], "val": evals["valid_1"]})
            .explode(["train", "val"])
            .reset_index(drop=True)
        ).assign(quantile=quantile)
        ...
        for pred_iter in pred_iters:
            ...
            if use_log:
                df_pred[f"pred_volume_{quantile}"] = np.expm1(df_pred[f"pred_volume_{quantile}"])
                df_pred[cfg["target"]] = np.expm1(df_pred[cfg["target"]])
            if pred_output:
                df_pred.to_csv(f"{pred_output}_p{quantile}_e{pred_iter}.csv", index=False)
        if model_output:
            estimator.save_model(f"{model_output}_p{quantile}.bin")

        df_eval_scores.append(df_eval_score)
        estimators.append(estimator)

    df_eval_scores = pd.concat(df_eval_scores)
    if eval_output:
        df_eval_scores.to_csv(f"{eval_output}.csv", index=False)
    if feature_output:
        pd.DataFrame({"features": all_features}).to_csv(f"{feature_output}.csv", index=False)

    return estimators, df_pred


def predict_lgb(df, quantiles, cfg, model_output, features=None):
    df = df.copy()
    df = as_categorical(df, cfg["cat_features"])
    if features:
